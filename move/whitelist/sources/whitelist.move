...
public fun create_whitelist(nft_id: ID, seal_policy_id: String, ctx: &mut TxContext): (Cap, Whitelist) {
    let wl = Whitelist {
        id: object::new(ctx),
        version: VERSION,
        nft_id,
...
entry fun create_whitelist_entry(nft_id: ID, seal_policy_id: String, ctx: &mut TxContext) {
    let (cap, wl) = create_whitelist(nft_id, seal_policy_id, ctx);
    share_whitelist(wl);
...
fun check_policy(caller: address, nft: &nft::nft::NFT, wl: &Whitelist): bool {
    ...
    // Check if caller is in the NFT owners list
    let mut i = 0;
